...
The image serves the API and the built frontend from one container, and runs as
a **non-root user so Chromium's sandbox stays enabled** — the common
`PUPPETEER_NO_SANDBOX=1` shortcut removes the main containment if a renderer is
compromised.
...
Chromium's sandbox is **on**; set `PUPPETEER_NO_SANDBOX=1` only for containers
that cannot provide the required namespaces. Concurrent renders are capped
(`PDF_MAX_CONCURRENT`, default 2).
